...
public class LeetCode1319 {
    ...
    public static int makeConnected(int n, int[][] connections) {
        ...
        for (int i=0;i<n;i++){
            ...
            maxEdge = Math.max(maxEdge,single);

        }
        // 减去最大连通图的点数
        int point = pointNum-1;
        // 图的个数  小于 冗余的边
        if(point<=(connections.length-maxEdge)){
    ...
    public static int makeConnected2(int n, int[][] connections) {
        ...
        for (int[] point:connections){
            int start = point[0];
            int end = point[1];
            edgeList.computeIfAbsent(start, k -> new LinkedList<Integer>());
            edgeList.computeIfAbsent(end, k -> new LinkedList<Integer>());
